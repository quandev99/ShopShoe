...
$(document).ready(()=>{
    ...
    try {
        const listProduct = $("#market-trend");
        ...
        categoryService.findAllCategories().then((data)=>{
            ...
            for(const idCate in data){
                ...
                const { name  } = element;
                listCate += `<li><a class="category-item" href="listProducts&Categories.html?id=${idCate}">${name}</a>
                    </li>`;
            }
        ...
        productService.findAllProducts(idProduct).then((data)=>{
            let productItem = "";
